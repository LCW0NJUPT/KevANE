...
The model files live in **`hf-model/` inside this checkout**. The installer checks for a complete local model first and skips the download when it is already there. Otherwise, it checks the [model repository](https://huggingface.co/flylcw/KevANE-0.6B) before downloading the required files into `hf-model/`. If you keep a converted model elsewhere, run `bash scripts/install.sh --model-dir /absolute/path/to/model` to use it without copying or downloading it. Networks that cannot reach huggingface.co directly can export a mirror first, e.g. `export HF_ENDPOINT=https://hf-mirror.com`; both the installer's repository check and `hf download` honour it.
...
- In the recorded comparison with the original FP32 path, the minimum hidden-state cosine similarity was **0.999058** across seven samples; **34 of 35** decisions kept the same top answer. The one changed answer was a near tie. The comparison was re-verified unchanged in the shipped `kevane-runtime` environment. See the [parity record](benchmarks/results/coreml_fp16_parity.json).
- With the model in place, run `conda activate kevane-runtime && python -m pytest tests/` to exercise the same parity, contract, and encoder checks. The model-dependent tests are skipped automatically on a fresh clone until `hf-model/` is populated.
