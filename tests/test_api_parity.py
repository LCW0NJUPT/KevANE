...
# 依赖下载完成的 hf-model/（scripts/install.sh）；新 clone 上跳过而不是收集报错。
if not (ROOT / "hf-model" / "kev-qwen3-0.6b-pointer-head.pt").is_file():
    pytest.skip("model files not present in hf-model/; run scripts/install.sh first",
